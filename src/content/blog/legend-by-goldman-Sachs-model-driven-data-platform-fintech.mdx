---
id: 13
slug: "finos-legend-by-goldman-Sachs-model-driven-data-platform-fintech"
title: "Open Source Data platform Legend by Goldman Sachs, A Model-Driven Platform Reshaping Financial Data"
...
---
id: 13
slug: "finos-legend-model-driven-data-platform-fintech"
title: "FINOS Legend: The Model-Driven Platform Reshaping Financial Data"
description: "A senior engineer's deep-dive into FINOS Legend - its architecture, Pure language, real-world fintech applications, and why model-driven data governance may be the most important infrastructure bet in financial services."
imageUrl: "/images/blog-post/post-13.webp"
imageAlt: "FINOS Legend platform architecture diagram"
pubDate: "March 11, 2026"
...
```mermaid
graph TD
    A[Legend Studio\nModel Authoring] --> B[Legend SDLC\nVersion Control]
    B --> C[Legend Depot\nArtifact Registry]
    C --> D[Legend Engine\nExecution Layer]
    D --> E[Legend Query\nSelf-Service UI]
    D --> F[Legend Services\nREST API Generation]
    D --> G[(Physical Stores\nOracle / Postgres / Kafka / Files)]

    style A fill:#b84a2e,color:#fff
    style D fill:#1a1814,color:#e8d5a3
    style G fill:#3a3530,color:#e8d5a3
```
...
```mermaid
sequenceDiagram
    participant A as Architect (Studio)
    participant S as SDLC + Depot
    participant E as Engine
    participant Q as Analyst (Query)
    participant Svc as Services (API)

    A->>S: Author Pure model + mapping + runtime; commit + review
    S->>E: Publish model version; Engine resolves
    Q->>E: Build query via drag-and-drop
    E->>E: Generate execution plan; push to physical store
    Q->>Svc: Promote query to production API
    Svc-->>E: Versioned REST endpoint backed by same model
```
...
```mermaid
graph LR
    A[Business Model\nPure classes + constraints] --> B[Mapping\nLogical-to-physical bindings]
    B --> C[Runtime\nEnvironment connections]
    C --> D[Execution Plan\nInspectable query tree + lineage]
...
flowchart LR
    ...
    style M fill:#b84a2e,color:#fff
    style Gov fill:#c8943a,color:#fff
```
...
| Snowflake / BigQuery | SQL storage + compute | Access control; external catalog | Analytical workloads; fast SQL |
...
```mermaid
graph TD
    A[Data Contracts\nFormal producer-consumer interfaces] --> D[Legend's\nModel-Driven Architecture]
    B[Semantic Data Layers\nSingle governed meaning layer] --> D
    C[AI-Ready Data\nStructured semantics for LLMs] --> D

    style D fill:#b84a2e,color:#fff
```
